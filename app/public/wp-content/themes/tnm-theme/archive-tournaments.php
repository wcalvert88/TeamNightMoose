    ...
    <div id="primary" class="content-area">
        <main id="main" class="site-main">
            ...
            <article id="post-<?php the_ID(); ?>" class="post-<?php the_ID(); ?> page type-page status-publish hentry">
                <a href="<? the_permalink(); ?>"><h2 class='post-title'><?php the_title(); ?></h2></a>
                <div class="entry-content">
                    <h3 class="post-subtitle"><?php 
                    $tournament_date = new DateTime(get_field('tournament_date')); 
                    $tournament_date_month = $tournament_date->format('M');
                    $tournament_date_day = $tournament_date->format('d');
                    $tournament_date_year = $tournament_date->format('Y');
                    // $tournament_date_time = $tournament_date->format('H:i A');
                    echo $tournament_date_month . " ". $tournament_date_day . ", " . $tournament_date_year;
                    ?></h3>
                    <?php the_content(); ?>
                </div><!-- .entry-content -->
